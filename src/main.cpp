...
// Copyright (c) 2013-2014 Cakecoin Developers
...
uint256 hashGenesisBlock("0xe8f9ccd8cec92e1410117d3562cbf1469832fca3111ab335b938f160c7c74ffb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cakecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cakecoin Signed Message:\n";
...
{
    // Cakecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else if (nHeight >= 30001 && nHeight <= 60000)
    {
    ...
    else if (nHeight >= 90001 && nHeight <= 150000)
    {
    ...
    {
        int64 nSubsidy = 870 * COIN;
        return nSubsidy + nFees;
    }
    else if (nHeight >= 200001 && nHeight <= 218000)
    {
        int64 nSubsidy = 500 * COIN;
        return nSubsidy + nFees;
    }
    else if (nHeight >= 218001 && nHeight <= 226000)
    {
        int64 nSubsidy = 2500 * COIN;
        return nSubsidy + nFees;
    }
    else if (nHeight >= 226001 && nHeight <= 251000)
    {
        int64 nSubsidy = 2000 * COIN;
        return nSubsidy + nFees;
    }
    else if (nHeight >= 251001 && nHeight <= 301000)
    {
        int64 nSubsidy = 1000 * COIN;
        return nSubsidy + nFees;
    }
    else if (nHeight >= 301001 && nHeight <= 601000)
    {
        int64 nSubsidy = 250 * COIN;
        return nSubsidy + nFees;
    }
    else if (nHeight >= 601000 && nHeight <= 1601000)
    {
...
static const int64 nTargetTimespan = 60 * 60; // 1hr target time
static const int64 nTargetSpacing = 60; // seconds per block target
static const int64 nInterval = nTargetTimespan / nTargetSpacing; //Aim to produce 60 blocks every 1 hours
...
unsigned int static GetNextWorkRequired_V3(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // thanks to RealSolid & WDC for the basis of this code, modified by Mammix2 for Cakecoin
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cakecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Cakecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Its all about the Cake!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x4b2065f7f39f68ae5779a45054de80faff89ce57f3d908987f12aab8b82e6249"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Cakecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CakecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CakecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CakecoinMiner(CWallet *pwallet)
{
    printf("CakecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cakecoin-miner");
    ...
    try { loop {
        ...
        printf("Running CakecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CakecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CakecoinMiner, pwallet));
}
